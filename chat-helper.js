...
// @author       yu-chenglong
// @license      MIT
// @homepage     https://github.com/yu-chenglong/GMScripts
...
(function () {
  ...
  // 加载中的旋转图标（与 refresh 相同，但添加旋转动画类）
  const ICON_SPINNER =
    '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" class="ch-spin"><path d="M23 4v6h-6M1 20v-6h6M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>';
  ...
  function initUI() {
    ...
    console.log('Chat Helper v1.0.0 started');
  }
    ...
    let check = () => {
      if (document.querySelector(currentSite.waitFor || 'body') && document.body) {
        if (initTemplateUrl()) {
          ...
          console.warn('Chat Helper: No template URL set');
          initUI();
        }
      } else {
