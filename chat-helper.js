...
// @version      8.8
// @description  Material Design quick reply manager for Shopee/Lazada
// @match        https://seller.shopee.cn/*
// @match        https://seller.shopee.com.my/*
// @match        https://seller.shopee.co.th/*
// @match        https://sellercenter-*.lazada-seller.cn/*
...
(function () {
    ...
    let templates = [], currentCategory = '', modalVisible = false, currentModal = null, currentSite = null, templateUrl = '';
    ...
    // ==================== TEMPLATE LOADING ====================
    function saveToCache(data) { GM_setValue(STORAGE_CACHE_KEY, JSON.stringify(data)); GM_setValue(STORAGE_CACHE_TIME_KEY, Date.now()); }
    function loadFromCache() { let cached = GM_getValue(STORAGE_CACHE_KEY); if (cached) try { templates = JSON.parse(cached); return true; } catch (e) { } return false; }

    function loadFromRemote(callback, silent = false) {
        if (!templateUrl) { if (!silent) showToast('Set template URL first'); callback?.(); return; }
        GM_xmlhttpRequest({
            method: 'GET', url: `${templateUrl}?_t=${Date.now()}`,
            headers: { 'Cache-Control': 'no-cache' },
            onload: (resp) => {
                if (resp.status === 200) {
                    try {
                        let data = JSON.parse(resp.responseText);
                        if (Array.isArray(data)) { templates = data; saveToCache(templates); if (!silent) showToast('Templates updated ✓'); callback?.(); return; }
                        throw new Error('Not an array');
                    } catch (e) { if (!silent) showToast('Invalid JSON'); }
                } else if (!silent) showToast(`HTTP ${resp.status}`);
                callback?.();
            },
            onerror: () => { if (!silent) showToast('Network error'); callback?.(); }
        });
    }

    function initTemplates(callback) {
        if (loadFromCache()) { callback?.(); loadFromRemote(() => { }, true); }
        else loadFromRemote(callback);
    }

    function refreshTemplates(callback) { loadFromRemote(() => { refreshModalUI(); callback?.(); }, false); }

    // ==================== UI HELPERS ====================
    function escapeHtml(s) { return s?.replace(/[&<>]/g, m => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[m])) || ''; }
    function getCategories() { return [...new Set(templates.map(t => t.category))].sort(); }
    function getTemplatesByCategory(cat) { return templates.filter(t => t.category === cat); }
    function onTemplateClick(content) { if (AUTO_FILL) fillInputBox(content); else copyToClipboard(content); closeModal(); }
    ...
    function renderCategories() {
        ...
        cats.forEach(cat => {
            ...
            div.onclick = () => { currentCategory = cat; renderCategories(); renderTemplates(); };
    ...
    function renderTemplates() {
        ...
        templatesContainer.innerHTML = items.length ? '' : `<div class="ch-empty">${ICONS.empty}<div>No templates, click refresh</div></div>`;
        items.forEach(tpl => {
            let div = document.createElement('div');
    ...
    function refreshModalUI() {
        let cats = getCategories();
        if (cats.length && (!currentCategory || !cats.includes(currentCategory))) currentCategory = cats[0];
    ...
    function buildModal() {
        let backdrop = document.createElement('div'); backdrop.className = 'ch-dialog-backdrop';
        let dialog = document.createElement('div'); dialog.className = 'ch-dialog';

        let header = document.createElement('div'); header.className = 'ch-dialog-header';
        let title = document.createElement('span'); title.className = 'ch-dialog-title'; title.textContent = 'Chat Helper';
        let closeBtn = document.createElement('button'); closeBtn.className = 'ch-dialog-close'; closeBtn.innerHTML = ICONS.close;
        closeBtn.onclick = closeModal;
        header.appendChild(title); header.appendChild(closeBtn);

        let body = document.createElement('div'); body.className = 'ch-dialog-body';
        categoriesContainer = document.createElement('div'); categoriesContainer.className = 'ch-categories';
        templatesContainer = document.createElement('div'); templatesContainer.className = 'ch-templates';
        body.append(categoriesContainer, templatesContainer);

        let footer = document.createElement('div'); footer.className = 'ch-dialog-footer';
        let footerLeft = document.createElement('div'); footerLeft.className = 'ch-footer-left';
        let footerRight = document.createElement('div'); footerRight.className = 'ch-footer-right';

        let changeUrlBtn = document.createElement('button'); changeUrlBtn.className = 'ch-settings-btn';
        changeUrlBtn.innerHTML = `${ICONS.settings} Change URL`;
        changeUrlBtn.onclick = (e) => { e.stopPropagation(); showUrlEditModal(); };

        let refreshBtn = document.createElement('button'); refreshBtn.className = 'ch-refresh-btn';
        refreshBtn.innerHTML = `${ICONS.refresh} Refresh`;
        refreshBtn.onclick = (e) => { e.stopPropagation(); if (confirm('Refresh from remote?')) refreshTemplates(() => refreshModalUI()); };

        let urlDisplay = document.createElement('span'); urlDisplay.className = 'ch-url-display';
        let displayUrl = templateUrl || 'No URL set';
        urlDisplay.title = displayUrl;
        urlDisplay.textContent = displayUrl.length > 55 ? displayUrl.substring(0, 52) + '...' : displayUrl;

        let cacheTime = GM_getValue(STORAGE_CACHE_TIME_KEY, 0);
        let cacheSpan = document.createElement('span'); cacheSpan.className = 'ch-cache-info';
    ...
    function openModal() { if (currentModal) closeModal(); currentModal = buildModal(); document.body.appendChild(currentModal); modalVisible = true; }
    function closeModal() { if (currentModal) currentModal.remove(); currentModal = null; modalVisible = false; }

    // ==================== INIT ====================
    function initUI() {
        if (document.querySelector('.ch-fab')) return;
        let fab = document.createElement('div'); fab.className = 'ch-fab'; fab.innerHTML = ICONS.fab; fab.title = 'Chat Helper';
    ...
    function waitForPage() {
        ...
        let check = () => {
            if (document.querySelector(currentSite.waitFor || 'body') && document.body) {
                ...
                else { console.warn('No template URL'); initUI(); }
