...
(function() {
    ...
    class DOMHelper {
        ...
        static triggerMouseEvent(element, eventType) {
            if (!element) {
    ...
    class CheckboxHandler {
        constructor(checkbox, trackingNumber) {
            this.checkbox = checkbox;
            this.trackingNumber = trackingNumber;
            this.currentWindow = DOMHelper.getWindow();
        }

        async check() {
            if (!this.checkbox) {
                console.log(`无法勾选复选框：元素不存在 (${this.trackingNumber})`);
                return false;
            }

            console.log(`开始勾选复选框: ${this.trackingNumber}`);
            this.checkbox.classList.add('debug-checkbox');

            try {
                const initialState = this.checkbox.checked;

                // 按顺序执行事件序列
                await this.performCheckSequence(initialState);

                // 等待一段时间让框架处理变更
                await DOMHelper.delay(20);

                // 验证最终状态
                const finalState = this.checkbox.checked;
                const isSuccess = finalState !== initialState;

                if (isSuccess) {
                    console.log(`✅ 成功勾选复选框: ${this.trackingNumber}`);
                    this.checkbox.classList.remove('debug-checkbox');
                    this.checkbox.classList.add('checked-checkbox');

                    setTimeout(() => {
                        this.checkbox.classList.remove('checked-checkbox');
                    }, 3000);
                } else {
                    console.log(`❌ 勾选失败: ${this.trackingNumber} (状态未改变)`);
                }

                return isSuccess;
            } catch (error) {
                console.error(`勾选复选框时出错 (${this.trackingNumber}):`, error);
                return false;
            }
        }
    ...
    class ShopeeOrderSelector {
        ...
        createModal() {
            ...
            searchButton.addEventListener('click', async () => {
                try {
                    ...
                    const trackingNumbers = input.value.split('\n').map(num => num.trim()).filter(num => num);
                    ...
                    this.notFoundNumbers = [...trackingNumbers];
                    this.results = { success: 0, failed: 0, skipped: 0 };

                    await this.searchAndCheck(trackingNumbers);
        ...
        async searchAndCheck(trackingNumbers) {
            ...
            // 查找包裹追踪号列的索引
            const headerRow = document.querySelector('table.eds-table__header thead tr');
            if (!headerRow) {
                console.log('未找到表格头部');
            ...
            rows.forEach(row => {
                ...
                trackingNumbers.forEach(num => {
                    if (trackingNumber.includes(num)) {
                        ...
                        // 从"未找到"列表中移除
                        const index = this.notFoundNumbers.indexOf(num);
                        if (index > -1) {
                            this.notFoundNumbers.splice(index, 1);
                        }
                    }
                });
            });
            ...
            for (const { row, trackingNumber } of matchingRows) {
                ...
                await this.processRow(row, trackingNumber);
                await DOMHelper.delay(500); // 处理间隔，避免过快
            }
        }

        async processRow(row, trackingNumber) {
            ...
            // 创建复选框处理器并尝试勾选
            const handler = new CheckboxHandler(checkbox, trackingNumber);
            const isSuccess = await handler.check();
        ...
        showResults() {
            // 显示结果统计
            let message = `处理完成!\n成功勾选: ${this.results.success}\n勾选失败: ${this.results.failed}\n未找到: ${this.notFoundNumbers.length}`;

            if (this.notFoundNumbers.length > 0) {
                message += `\n\n未找到的追踪号 (${this.notFoundNumbers.length}):\n${this.notFoundNumbers.join('\n')}`;
            }
    ...
    // 初始化并运行应用
    const app = new ShopeeOrderSelector();
    app.run();
})();